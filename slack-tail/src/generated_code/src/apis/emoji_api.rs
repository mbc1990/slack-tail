...
pub trait EmojiApi {
    fn emoji_list(&self, token: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>>;
}
...
impl<C: hyper::client::Connect>EmojiApi for EmojiApiClient<C> {
    fn emoji_list(&self, token: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>> {
        ...
        let uri_str = format!("{}/emoji.list?{}", configuration.base_path, query_string);
