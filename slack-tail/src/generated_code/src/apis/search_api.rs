...
pub trait SearchApi {
    fn search_messages(&self, query: &str, token: &str, sort_dir: &str, sort: &str, count: i32, highlight: bool, page: i32) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>>;
}
...
impl<C: hyper::client::Connect>SearchApi for SearchApiClient<C> {
    fn search_messages(&self, query: &str, token: &str, sort_dir: &str, sort: &str, count: i32, highlight: bool, page: i32) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>> {
        ...
        let uri_str = format!("{}/search.messages?{}", configuration.base_path, query_string);
