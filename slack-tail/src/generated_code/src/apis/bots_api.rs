...
pub trait BotsApi {
    fn bots_info(&self, token: &str, bot: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>>;
}
...
impl<C: hyper::client::Connect>BotsApi for BotsApiClient<C> {
    fn bots_info(&self, token: &str, bot: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>> {
        ...
        let query_string = {
            ...
            query.append_pair("bot", &bot.to_string());
            for (key, val) in &auth_query {
                query.append_pair(key, val);
            }
            query.finish()
        };
        let uri_str = format!("{}/bots.info?{}", configuration.base_path, query_string);
