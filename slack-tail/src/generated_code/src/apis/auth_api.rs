...
pub trait AuthApi {
    fn auth_revoke(&self, token: &str, test: bool) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>>;
    fn auth_test(&self, token: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>>;
}
...
impl<C: hyper::client::Connect>AuthApi for AuthApiClient<C> {
    fn auth_revoke(&self, token: &str, test: bool) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>> {
        ...
        let query_string = {
            ...
            query.append_pair("test", &test.to_string());
        ...
        let uri_str = format!("{}/auth.revoke?{}", configuration.base_path, query_string);
    ...
    fn auth_test(&self, token: &str) -> Box<Future<Item = ::std::collections::HashMap<String, Value>, Error = Error<serde_json::Value>>> {
        ...
        let uri_str = format!("{}/auth.test?{}", configuration.base_path, query_string);
